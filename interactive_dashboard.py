...
def cached_identify_aftershocks(_df, mainshock_mag, aftershock_window, distance_window):
    print("DEBUG: cached_identify_aftershocks START")
    analyzer = EarthquakeChainAnalyzer(_df.copy())
    print(f"DEBUG: main_shocks count: {(analyzer.df['mag'] >= mainshock_mag).sum()}")
    try:
        analyzer.identify_aftershocks(mainshock_mag, aftershock_window, distance_window)
        print("DEBUG: identify_aftershocks END")
    except Exception as e:
        print(f"DEBUG: identify_aftershocks ERROR: {e}")
...
class EarthquakeDashboard:
    ...
    def show_chain_analysis(self):
        ...
        **What is this?**
        ...
        st.write("DEBUG: Before cached_identify_aftershocks")
        try:
            df_with_aftershocks, omori_params = cached_identify_aftershocks(self.analyzer.df, mainshock_mag, aftershock_window, distance_window)
            st.write("DEBUG: After cached_identify_aftershocks")
            st.write(f"df_with_aftershocks shape: {df_with_aftershocks.shape}")
            st.write(f"omori_params: {type(omori_params)} | {omori_params if omori_params is not None else 'None'}")
            self.chain_analyzer.df = df_with_aftershocks
            self.chain_analyzer.omori_params = omori_params
            col1, col2 = st.columns(2)
            with col1:
                st.metric('Number of Main Shocks', len(self.chain_analyzer.df[self.chain_analyzer.df['mag'] >= mainshock_mag]))
            with col2:
                if 'is_aftershock' in self.chain_analyzer.df.columns:
                    ...
                    # Debug: show aftershock indices and first few aftershock events
                    st.write(f"Aftershock indices: {self.chain_analyzer.df[self.chain_analyzer.df['is_aftershock']].index.tolist()[:10]}")
